...
- [ ] **FUENTE NUEVA PENDIENTE: segunda hoja de Torneos Municipales 2017, Nº EQUIPO 120202 (MdL, 17 jugadores).** Iván informa de su existencia; **no está en el repositorio** (`docs/Fichas` sólo tiene la 119823, de 20 jugadores) ni apareció al revisar 1157 PDF de `Downloads`, `Desktop/HEP Equipo Iván` y `Desktop/escritorio` el 04/08/2026. **Hace falta el PDF para cruzarla.** Dos cosas a resolver cuando llegue:
  - **Riesgo de pérdida silenciosa en el pipeline**: `consolidar_fichas.js` agrupa por año + equipo + competición (liga/torneo) y dentro de cada grupo se queda con la hoja de MÁS jugadores. Dos hojas de torneo del mismo año y equipo caen en la misma clave, así que la de 17 se descartaría entera y se perdería a quien sólo esté en ella. **No hay riesgo de doble conteo** (la unión de nombres deduplica), sólo de omisión. Habría que agrupar también por Nº EQUIPO para que las dos inscripciones sumen, como ya hacen liga y torneo.
  - **Predicción comprobable**: en la matriz hay 4 personas con 2016 que NO están en la 119823 (Alan Venegas, Carlos Pérez Núñez, Juan Carlos Blázquez y Pupo). Si la 120202 las contiene, quedaría explicado de dónde salían y la temporada 2016/17 quedaría cerrada por dos fuentes independientes.

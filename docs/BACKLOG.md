...
- [ ] **Añadir a `rivales_jdm.json` el tercer equipo de 2020/21 (`#149233`).** El bloque de rivales lo descartó por 0 % de coincidencia de marcadores, pero eso sólo probaba que no era el equipo del Excel: **la clasificación interna del Excel lo lista junto a `MDL` y `MDA` en el mismo grupo, así que sí era del club**. Faltan sus 14 partidos.
